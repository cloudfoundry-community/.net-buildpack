...
module NETBuildpack::Repository
  ...
  describe ConfiguredItem do
    ...
    before do
      NETBuildpack::Repository::RepositoryIndex.stub(:new).and_return(double('repository index', find_item: [RESOLVED_VERSION, RESOLVED_URI]))
    end
    ...
    it 'drives the version validator block if supplied' do
      ConfiguredItem.find_item(
        ...
        expect(version).to eq(NETBuildpack::Util::TokenizedVersion.new('1.7.0'))
      end
    end
